...
class UploadChunkController extends Controller
{
    ...
    public function createExampleDoc(Request $request)
    {
        try {
            $chunkIndex = $request->input('chunk_index');
            $totalChunks = $request->input('total_chunks');
            ...
            if (
                !$file || is_null($chunkIndex)
                || is_null($totalChunks) || is_null($originalName)
            ) {
                return Response::handler(400, 'Bad Request', [], [], ['file' => ['Missing upload information.']]);
            }

            File::storeChunk($file, $chunkIndex, '1');

            if ((int) $chunkIndex+1 === (int) $totalChunks) {
                $storedPath = File::mergeChunks('1',$originalName, 'example_docs');
    ...
    public function deleteExampleDoc(Request $request)
    {
        try {
            $file = $request->input('file');
            $path = storage_path('app/public/' . $file);
